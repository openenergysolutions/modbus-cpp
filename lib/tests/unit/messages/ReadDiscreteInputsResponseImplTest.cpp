...
TEST_CASE("ReadDiscreteInputsResponseImpl")
{
    ...
    SECTION("Parse")
    {
        ...
        ReadDiscreteInputsRequest request{starting_address, qty_of_bits};
        ...
        SECTION("When proper response, then parse it properly") {
            std::array<uint8_t, 4> proper_response{{
                ...
                0b01010101, 0b11110000, // Discrete input values
            }};
            ser4cpp::rseq_t buffer{proper_response.data(), proper_response.size()};

            auto result = ReadDiscreteInputsResponseImpl::parse(request_impl, buffer);
        ...
        SECTION("When proper response for request with number of bits not a multiple of 8, then parse it properly")
        {
            ...
            ReadDiscreteInputsRequest request_not_multiple_of_8{starting_address, qty_of_bits_not_multiple_of_8};
            ReadDiscreteInputsRequestImpl request_impl_not_multiple_of_8{request_not_multiple_of_8};
            std::array<uint8_t, 5> proper_response{{
                0x02,       // Function code
                0x03,       // Number of bytes
                0b01010101, 0b11110000, 0b00000001 // Discrete input values
            }};
            ser4cpp::rseq_t buffer{proper_response.data(), proper_response.size()};

            auto result = ReadDiscreteInputsResponseImpl::parse(request_impl_not_multiple_of_8, buffer);

...
TEST_CASE("MbapMessage")
{
    ...
    loopser::rseq_t raw_request{raw_request_data.data(), raw_request_data.size()};
    ...
    SECTION("When build message of size 0, then appropriately build the message")
    {
        ...
        loopser::rseq_t expected_message{expected_message_data.data(), expected_message_data.size()};

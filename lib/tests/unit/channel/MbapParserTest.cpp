...
TEST_CASE("MbapParser")
{
    ...
    {
        loopser::rseq_t buffer{complete_message.data(), complete_message.size()};
    ...
    SECTION("When receive two complete messages in one call, then report both messages")
    {
        ...
        loopser::rseq_t buffer{two_messages.data(), two_messages.size()};
    ...
    {
        loopser::rseq_t buffer{complete_message.data(), complete_message.size() - 1};
        ...
        {
            loopser::rseq_t buffer{complete_message.data() + complete_message.size() - 1, 1};
    ...
    SECTION("When reset, then stop parsing and reset internal state")
    {
        ...
        loopser::rseq_t buffer{incomplete_message.data(), incomplete_message.size()};
        parser.parse(buffer);

        parser.reset();

        buffer = loopser::rseq_t{complete_message.data(), complete_message.size()};
        parser.parse(buffer);
        REQUIRE(sink.get_num_messages() == 1);
        check_complete_message(sink.get_messages()[0]);
    }
    ...
    SECTION("When receive wrong protocol identifier, then reject message")
    {
        ...
        loopser::rseq_t buffer{wrong_protocol_id_message.data(), wrong_protocol_id_message.size()};
    ...
    SECTION("When length is 0, then reject message")
    {
        ...
        loopser::rseq_t buffer{length_zero_message.data(), length_zero_message.size()};
    ...
    SECTION("When length is greater than the maximum, then go to end of stream, but only report maximum size")
    {
        ...
        loopser::rseq_t buffer{length_max_message.data(), length_max_message.size()};
